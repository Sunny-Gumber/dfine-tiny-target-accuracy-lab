...
type FaceDetectorInstance = {
  ...
  setOptions: (options: { runningMode?: RunningMode; minDetectionConfidence?: number }) => Promise<void>;
  close?: () => void;
};
...
export default function FaceLab() {
  ...
  const detectorPromiseRef = useRef<Promise<FaceDetectorInstance> | null>(null);
  ...
  const ensureDetector = useCallback(async () => {
    ...
    const promise = (async () => {
      const moduleUrl: string = TASKS_URL;
      const visionModule = (await import(/* @vite-ignore */ moduleUrl)) as unknown as VisionModule;
      const vision = await visionModule.FilesetResolver.forVisionTasks(WASM_URL);

      try {
        const detector = await visionModule.FaceDetector.createFromOptions(vision, {
  ...
  const setDetectorMode = useCallback(async (mode: RunningMode) => {
  ...
  const drawAndCrop = useCallback(
    (
      ...
      detections.forEach((detection, index) => {
        ...
        const marginX = boxWidth * CROP_MARGIN;
        const marginY = boxHeight * CROP_MARGIN;
        const centerX = x + boxWidth / 2;
        const centerY = y + boxHeight / 2;
        const side = Math.min(
          Math.max(boxWidth + marginX * 2, boxHeight + marginY * 2),
          width,
          height,
        );
    ...
    <main className="page-shell face-lab">
      ...
      <footer className="footer-card">
        ...
        <div>
          ...
          <p>Each face is cropped with a small margin so we can inspect the actual image available to later analytics.</p>
        </div>
        <div>
